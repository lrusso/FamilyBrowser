package ar.com.lrusso.familybrowser.Activity;
...
import ar.com.lrusso.familybrowser.Browser.BrowserController;
import ar.com.lrusso.familybrowser.Database.Record;
import ar.com.lrusso.familybrowser.Database.RecordAction;
import ar.com.lrusso.familybrowser.DynamicGrid.DynamicGridView;
import ar.com.lrusso.familybrowser.Service.ClearService;
import ar.com.lrusso.familybrowser.Service.HolderService;
...
import ar.com.lrusso.familybrowser.View.*;
...
public class BrowserActivity extends Activity implements BrowserController
    ...
    @Override public void onCreate(Bundle savedInstanceState)
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP)
        	{
            ActivityManager.TaskDescription description = new ActivityManager.TaskDescription(getString(R.string.app_name),BitmapFactory.decodeResource(getResources(), R.drawable.ic_launcher),getResources().getColor(R.color.background_dark));
        ...
        switcherPanel.setStatusListener(new SwitcherPanel.StatusListener()
            ...
            @Override public void onCollapsed() {inputBox.clearFocus();}
        	});
    ...
    private void dispatchIntent(Intent intent)
        ...
        if (intent != null && intent.hasExtra(IntentUnit.OPEN))
        	{
        	// From HolderActivity's menu
            pinAlbums(intent.getStringExtra(IntentUnit.OPEN));
        	}
        else if (intent != null && intent.getAction() != null && intent.getAction().equals(Intent.ACTION_WEB_SEARCH))
        	{
    ...
    private void initOmnibox()
        ...
        inputBox.setOnTouchListener(new SwipeToBoundListener(omnibox, new SwipeToBoundListener.BoundCallback()
            ...
            @Override public boolean canSwipe()
            	{
                boolean ob = false;
            ...
            @Override public void onBound(boolean canSwitch, boolean left)
            	{
    ...
    private void initBHList(final NinjaRelativeLayout layout, boolean update)
        ...
        if (layout.getFlag() == BrowserUnit.FLAG_BOOKMARKS)
            ...
            Collections.sort(list, new Comparator<Record>()
            	{
                @Override public int compare(Record first, Record second)
                	{
    ...
    private synchronized void addAlbum(String title, final String url, final boolean foreground, final Message resultMsg)
        ...
        if (!foreground)
        	{
            ViewUnit.bound(this, webView);
        	if (isBlacklisted(url,webView)==false)
    			{
        		webView.loadUrl(url);
    			}
            webView.deactivate();
    ...
    @Override public synchronized void showAlbum(AlbumController controller, boolean anim, final boolean expand, final boolean capture)
    	{
    ...
    private synchronized void updateAlbum(String url)
        ...
        if (currentAlbumController instanceof NinjaWebView)
        	{
        	if (isBlacklisted(url,((NinjaWebView) currentAlbumController))==false)
        		{
    ...
    @Override public synchronized void removeAlbum(AlbumController controller)
    	{
    ...
    @Override public void updateAutoComplete()
    	{
        RecordAction action = new RecordAction(this);
        action.open(false);
    ...
    @Override public void updateBookmarks()
    	{
        if (currentAlbumController == null || !(currentAlbumController instanceof NinjaWebView))
        	{
    ...
    @Override public void updateInputBox(String query)
    	{
        if (query != null)
        	{
    ...
    @Override public synchronized void updateProgress(int progress)
    	{
    ...
    @Override public void showFileChooser(ValueCallback<Uri[]> filePathCallback, WebChromeClient.FileChooserParams fileChooserParams)
    	{
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP)
        	{
    ...
    @Override public void onCreateView(WebView view, final Message resultMsg)
    	{
        if (resultMsg == null)
        	{
            return;
    ...
    @Override public boolean onShowCustomView(View view, int requestedOrientation, WebChromeClient.CustomViewCallback callback)
    	{
        return onShowCustomView(view, callback);
    	}

    @Override public boolean onShowCustomView(View view, WebChromeClient.CustomViewCallback callback)
    	{
        if (view == null)
        	{
            return false;
    ...
    @Override public void onLongPress(String url)
    	{
    ...
    private boolean showOverflow()
        ...
        if (currentAlbumController != null && currentAlbumController instanceof NinjaRelativeLayout)
        	{
            stringList.remove(array[0]); // Add to home
            stringList.remove(array[2]); // Find in page
            stringList.remove(array[3]); // Share
        	}
        	...
        	listView.setOnItemClickListener(new AdapterView.OnItemClickListener()
        		...
        		@Override public void onItemClick(AdapterView<?> parent, View view, final int position, long id)
        			...
        			if (s.equals(array[0])) // Add to home
        				{
        				NinjaWebView ninjaWebView = (NinjaWebView) currentAlbumController;
        				RecordAction action = new RecordAction(BrowserActivity.this);
        				action.open(true);
        			...
        			else if (s.equals(array[4])) // Privacy policy
    					{
        				clickInPrivacy();
    					}
        			else if (s.equals(array[5])) // About
    					{
        				clickInAbout();
    					}
    ...
    private boolean prepareRecord()
        ...
        if (title == null || title.isEmpty() || url == null || url.isEmpty() || url.startsWith(BrowserUnit.URL_SCHEME_ABOUT) || url.startsWith(BrowserUnit.URL_SCHEME_MAIL_TO) || url.startsWith(BrowserUnit.URL_SCHEME_INTENT))
        	{
            return false;
        	}
        return true;
	...
	private void loadAssetToList(String file, List<String> destiny)	
		{
		String textFile = loadAssetTextAsString(file);
		BufferedReader rdr = null;
		try
			{
			rdr = new BufferedReader(new StringReader(textFile));
	...
	public boolean isBlacklisted(String url, WebView webview)
        ...
        for (int i=0;i<blacklist.size();i++)
    		{
        	if (url.toLowerCase().contains(blacklist.get(i)))
    			{
        		blacklisted = true;
        		i=blacklist.size()+1;
    			}
        ...
        for (int i=0;i<words.size();i++)
    		{
        	if (url.toLowerCase().contains(words.get(i)))
    			{
        		blacklisted = true;
        		i=words.size()+1;
    			}
